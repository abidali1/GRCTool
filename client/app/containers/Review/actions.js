...
export const fetchControlReviews = slug => {
  return async (dispatch, getState) => {
    try {
      ...
      const {
        ...
        totalReviews,
        totalSummary
      } = getControlReviewsSummary(response.data.reviews);

      dispatch({
        type: FETCH_CONTROL_REVIEWS,
        payload: {
...
export const addControlReview = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const control = getState().control.storeControl;
      ...
      if (response.data.success === true) {
        dispatch(success(successfulOptions));
        dispatch(fetchControlReviews(control.slug));
...
export const getControlReviewsSummary = reviews => {

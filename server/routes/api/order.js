...
router.get('/:orderId', auth, async (req, res) => {
  try {
    ...
    let order = {
      ...
      created: orderDoc.created,
      totalTax: 0,
      controls: orderDoc?.cart?.controls,
...
router.delete('/cancel/:orderId', auth, async (req, res) => {
  try {
    ...
    increaseQuantity(foundCart.controls);
...
router.put('/status/item/:itemId', auth, async (req, res) => {
  try {
    ...
    const foundCart = await Cart.findOne({ 'controls._id': itemId });
    const foundCartControls = foundCart.controls.find(p => p._id == itemId);
    ...
    if (status === 'Cancelled') {
      await Control.updateOne(
        { _id: foundCartControls .control },
        { $inc: { quantity: foundCartControls .quantity } }
      );

      const cart = await Cart.findOne({ _id: cartId });
      const items = cart.controls.filter(item => item.status === 'Cancelled');

      // All items are cancelled => Cancel order
      if (cart.controls.length === items.length) {
        await Order.deleteOne({ _id: orderId });
...
const increaseQuantity = controls => {
  let bulkOptions = controls.map(item => {
    return {
      updateOne: {
        filter: { _id: item.control },
